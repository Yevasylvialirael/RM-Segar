...
export default function App() {
  ...
  const handleSendMessage = async (e?: React.FormEvent, initialPrompt?: string) => {
    ...
    try {
      ...
      const aiResponse = response.text || "Maaf, saya sedang tidak fokus. Bisa ulangi?";
  ...
  useEffect(() => {
    ...
    const onboardingCompleted = localStorage.getItem('rm_segar_onboarding_completed');
  ...
  const completeOnboarding = () => {
    localStorage.setItem('rm_segar_onboarding_completed', 'true');
  ...
  const onboardingSteps = [
    ...
    },
    {
      ...
      position: "top-categories",
      button: "Lanjut"
    },
    {
      title: "Menu Utama",
      description: "Gunakan navigasi ini untuk berpindah antara Beranda, Favorit, dan Profilmu.",
      position: "bottom-nav",
      button: "Lanjut"
    },
    {
      title: "Keranjang Belanja",
      description: "Klik tombol ini untuk melihat pesananmu dan melakukan checkout via WhatsApp.",
      position: "bottom-cart",
      button: "Selesai"
    }
  ];
  ...
  const renderOnboarding = () => {
    ...
    const getArrowClass = () => {
      switch (step.position) {
        case 'top-search': return 'bottom-full mb-2 left-1/2 -translate-x-1/2 rotate-180';
        case 'top-categories': return 'bottom-full mb-2 left-1/2 -translate-x-1/2 rotate-180';
        case 'bottom-nav': return 'top-full mt-2 left-1/2 -translate-x-1/2';
        case 'bottom-cart': return 'top-full mt-2 left-1/2 -translate-x-1/2';
    ...
    return (
      <motion.div 
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-[2px] flex flex-col items-center p-6"
      >
        ...
        {/* Highlight Area (Visual only) */}
        {step.position !== 'center' && (
          <motion.div 
            layoutId="highlight"
            className={`absolute border-2 border-orange-500 rounded-2xl pointer-events-none ${
              step.position === 'top-search' ? 'top-[110px] left-6 right-6 h-16' :
              step.position === 'top-categories' ? 'top-[190px] left-0 right-0 h-14' :
              step.position === 'bottom-nav' ? 'bottom-0 left-0 right-0 h-20 rounded-t-[32px]' :
              step.position === 'bottom-cart' ? 'bottom-4 left-1/2 -translate-x-1/2 w-20 h-20 rounded-full' : ''
            }`}
          />
        )}
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`absolute left-6 right-6 bg-white rounded-[32px] p-8 shadow-2xl flex flex-col items-center text-center ${
              step.position === 'top-search' ? 'top-44' :
              step.position === 'top-categories' ? 'top-64' :
              step.position === 'bottom-nav' ? 'bottom-32' :
              step.position === 'bottom-cart' ? 'bottom-40' :
              'top-1/2 -translate-y-1/2'
            }`}
          >
            {/* Arrow */}
            <div className={`absolute w-4 h-4 bg-white rotate-45 ${getArrowClass()}`} />
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex gap-1.5">
                {onboardingSteps.map((_, idx) => (
                  <div 
                    key={idx}
                    className={`h-1 rounded-full transition-all duration-300 ${idx === onboardingStep ? 'w-4 bg-orange-500' : 'w-1 bg-stone-200'}`}
                  />
                ))}
              </div>
  ...
  const renderHome = () => (
    ...
    >
      ...
      {/* Categories */}
      <section className="px-6 overflow-x-auto no-scrollbar flex gap-4 py-4">
        {categories.map((cat) => (
          <button
            key={cat.name}
          ...
          >
            ...
            <span className={`text-xs font-bold ${activeCategory === cat.name ? 'text-stone-900' : 'text-stone-500'}`}>
              {cat.name}
            </span>
          </button>
        ))}
        ...
        <div className="space-y-4">
          {categoryItems.map((item) => (
            <motion.div 
              layout
              key={item.id}
              className="bg-white p-3 rounded-3xl flex gap-4 shadow-sm border border-stone-50"
            >
              ...
              <div className="flex-grow flex flex-col justify-center py-1">
                ...
                <p className="text-xs text-stone-400 mb-3 line-clamp-1">{item.description}</p>
          ...
          <div className="space-y-3">
            ...
            {user ? (
              ...
              <div className="pt-4 space-y-6">
                <div className="bg-white p-8 rounded-[32px] shadow-sm border border-stone-50 space-y-6">
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] pb-32 overflow-x-hidden relative">
      <AnimatePresence>
        {isLoading && (
          <motion.div 
            initial={{ opacity: 1 }}
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-100 px-8 py-4 flex justify-between items-center z-40 rounded-t-[32px] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        <button 
        ...
        {isCartOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed inset-x-0 bottom-0 h-[85vh] bg-white rounded-t-[40px] shadow-2xl z-50 flex flex-col"
            >
        ...
        {isChatOpen && (
          <>
            ...
            <motion.div 
              ...
              exit={{ y: '100%' }}
              transition={{ type: 'spring', damping: 25, stiffness: 200 }}
              className="fixed inset-x-0 bottom-0 h-[90vh] bg-[#F8F9FB] rounded-t-[40px] shadow-2xl z-[80] flex flex-col overflow-hidden"
            >
              {/* Header */}
                ...
                <div className="px-6 pb-4 flex gap-2 overflow-x-auto no-scrollbar">
                  {["Rekomendasi mie", "Menu nasi favorit", "Minuman segar", "Menu paling pedas"].map(suggestion => (
                    <button 
                      ...
                      className="whitespace-nowrap px-4 py-2 bg-white border border-stone-100 rounded-full text-xs font-bold text-stone-600 shadow-sm active:scale-95 transition-all"
                    >
                      {suggestion}
                    </button>
                  ))}
              ...
              <div className="bg-white p-6 border-t border-stone-100">
                <form onSubmit={handleSendMessage} className="relative flex items-center gap-3">
                  <input 
                    ...
                    className="flex-grow bg-stone-50 border-none rounded-2xl py-4 px-6 pr-14 text-sm text-stone-900 placeholder:text-stone-400 focus:ring-2 focus:ring-orange-500/20 transition-all"
                  />
                  <button 
                    type="submit"
